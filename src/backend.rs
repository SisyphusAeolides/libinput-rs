...
impl TrackedDevice {
    ...
    unsafe fn refresh_generic_palms(&mut self, time_usec: u64) {
        ...
        for slot in &mut self.mt_slots {
            ...
            if began_this_frame {
                slot.initial_x = slot.x;
                slot.initial_y = slot.y;
            }
...
impl BackendState {
        ...
        for fd in fds {
            ...
            loop {
                ...
                for followup in followups {
                    match followup {
                        DwtFollowup::SyncKeyboard { source_fd } => {
                            self.apply_dwt_keyboard_sync(source_fd, ctx);
                        }
    ...
    unsafe fn process_device_batch(
        ...
        for event in batch {
            ...
            if event.event_type() == EventType::SYNCHRONIZATION && event.code() == 3 {
                followups.push(DwtFollowup::SyncKeyboard { source_fd: fd });
            }
    ...
    fn apply_dwt_keyboard_sync(&mut self, source_fd: RawFd, ctx: *mut LibinputContext) {
        let Ok(state) = self.devices.get(&source_fd).unwrap().device.get_key_state() else {
            return;
        };

        let active_keys: std::collections::HashSet<u16> = state.iter().map(|k| k.0).collect();
        let mut keys_released = 0;

        for touchpad in self.devices.values_mut() {
            if touchpad.dwt.paired_keyboards.contains(&source_fd) {
                let initial = touchpad.dwt.pressed_keys.len();
                touchpad
                    .dwt
                    .pressed_keys
                    .retain(|code| active_keys.contains(code));
                keys_released += initial - touchpad.dwt.pressed_keys.len();
            }
        }
        ...
        if keys_released > 0 {
            unsafe {
                crate::emit_debug_log(
                    ctx,
                    &format!("DWT synchronized dropped keys: {}", keys_released),
                );
            }
        }
    }
    ...
    unsafe fn apply_dwt_keyboard_event(
        ...
        for target_fd in targets {
            ...
            if first_key {
                if !touchpad.dwt.modifier_keys.is_empty() {
                    continue;
                }

                // don't trigger if we have an ongoing action that
                // can't possibly be an accidental touch.
                let mut pointer_motion_active = false;
                let active_slots = touchpad.mt_slots.iter().filter(|s| s.active).count();
                if active_slots == 1 {
                    for slot in &touchpad.mt_slots {
                        if !slot.active
                            || matches!(
                                slot.edge_state,
                                EdgeScrollState::Active | EdgeScrollState::New
                            )
                        {
                            continue;
                        }
                        let started = slot.began_usec;
                        if started != 0 {
                            let elapsed = time_usec.saturating_sub(started);
                            if elapsed >= 500_000 {
                                let moved_mm = touchpad_delta_mm(
                                    slot.x - slot.initial_x,
                                    (*touchpad.lib_device).abs_x_resolution,
                                    (*touchpad.lib_device).abs_x_range.or(touchpad.abs_x_range),
                                    (*touchpad.lib_device).width_mm,
                                )
                                .hypot(touchpad_delta_mm(
                                    slot.y - slot.initial_y,
                                    (*touchpad.lib_device).abs_y_resolution,
                                    (*touchpad.lib_device).abs_y_range.or(touchpad.abs_y_range),
                                    (*touchpad.lib_device).height_mm,
                                ));
                                if moved_mm >= 20.0 {
    ...
    unsafe fn expire_dwt_timeouts(&mut self) {
        let mut actual_pressed = std::collections::HashSet::new();
        for td in self.devices.values() {
            if td.is_keyboard {
                if let Some(keys) = td.device.cached_state().key_vals() {
                    for key in keys.iter() {
                        actual_pressed.insert(key.0);
                    }
                }
            }
        }
        for touchpad in self.devices.values_mut() {
            if !touchpad.dwt.pressed_keys.is_empty() {
                touchpad
                    .dwt
                    .pressed_keys
                    .retain(|code| actual_pressed.contains(code));
            }
        }
